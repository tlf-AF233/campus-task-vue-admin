    ...
    >
      ...
      <el-table-column label="操作">
        <div slot-scope="scope">
          <el-button v-if="!scope.row.selected" @click="addTrainee(scope.row.courseId)" size="small" type="primary" round>选课</el-button>
          <el-button v-if="scope.row.selected" @click="deleteTrainee(scope.row.courseId)" size="small" type="danger" round>退选</el-button>
          <!-- <el-button
...
export default {
  ...
  methods: {
    ...
    addTrainee(id) {
      this.req({
        url: `/api/course/v1/course/addTrainee?id=${id}`,
        method: 'POST'
      }).then(res => {
        if (res.code === 200) {
          this.$message({
    ...
    searchCourse() {
      ...
      this.req({
        url: '/api/course/v1/course/searchByStudent',
        data: {
